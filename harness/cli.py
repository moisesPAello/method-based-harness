...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    # When --force is about to overwrite a pre-existing settings.json that was NOT
    # produced by this tool, warn loudly — user settings would be silently lost.
    # (Unlike CLAUDE.md, settings.json gets a plain write, not a marked-block merge.)
    _SETTINGS_MARKER = "Generated by method-based-harness"
    settings_rel = ".claude/settings.json"
    settings_path = root / settings_rel
    if args.force and settings_path.exists() and settings_rel in result.files:
        try:
            existing_settings = settings_path.read_text()
        except OSError:
            existing_settings = ""
        if _SETTINGS_MARKER not in existing_settings:
            log("init: WARNING — .claude/settings.json already exists and does NOT appear to be a "
                "previous harness render. Overwriting it will replace your personal settings.")
            log("      Tip: move personal settings to .claude/settings.local.json "
                "(Claude Code merges both files).")
